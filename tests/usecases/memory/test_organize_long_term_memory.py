...
@pytest.mark.anyio
async def test_organize_long_term_memory_invokes_consolidation_service(
    mocker: Any,
) -> None:
    ...
    handler = OrganizeLongTermMemoryHandler(
        memory_store,
        uow,
    ...
    result = await handler.handle(
        OrganizeLongTermMemoryCommand(reference_time=datetime(2026, 5, 19, tzinfo=UTC))
    )
...
@pytest.mark.anyio
async def test_organize_long_term_memory_reports_failures(
    mocker: Any,
) -> None:
    ...
    result = await handler.handle(OrganizeLongTermMemoryCommand())
...
@pytest.mark.anyio
async def test_organize_long_term_memory_does_not_use_run_repository(
    mocker: Any,
) -> None:
...
@pytest.mark.anyio
async def test_organize_long_term_memory_consolidates_sqlite_raw_logs(
