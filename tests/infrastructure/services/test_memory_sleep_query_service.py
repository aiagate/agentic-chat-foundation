"""Tests for the memory sleep query service."""
...
async def test_list_pending_targets_groups_by_day_and_sorts_logs(
    ...
    query = mocker.Mock()
    query.list_memory_sleep_source_user_ids = mocker.AsyncMock(return_value=Ok(["u1"]))
    query.get_memory_sleep_source_items = mocker.AsyncMock(
        side_effect=[
            Ok(
                [
    ...
    service = MemorySleepQueryService()
    ...
    query.list_memory_sleep_source_user_ids.assert_awaited_once_with(
        until=datetime(2026, 5, 19, 0, 0, tzinfo=UTC),
        limit=10000,
    )
    assert query.get_memory_sleep_source_items.await_count == 2
...
async def test_list_pending_targets_raises_on_repository_error(
    ...
    query = mocker.Mock()
    query.list_memory_sleep_source_user_ids = mocker.AsyncMock(
        return_value=Err(
            RepositoryError(
