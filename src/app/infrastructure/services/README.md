...
Agent workflowの状態はPostgreSQLへ移行済みであり、旧tool store/router/notifierは削除した。`memory_sleep_query_service.py` は `src/app/infrastructure/queries` 配下へ移動済み。
...
| `agent_inference_context.py` | memory、profile、永続tool結果からLLM入力を組み立てる。 | `src/app/container.py`, `application/agent/turn_runner.py` | 推論前の入力集約Service。 |
...
| `memory_service.py` | `IMemoryService` の実装。検索コンテキスト取得と memory document の読み取りに専念する。 | `src/app/container.py`, `application/agent/turn_runner.py`, `tests` | read-only に整理済み。初期 agent プロファイルの作成は write 側の bootstrap に委譲した。 |
...
- tool call/result/leaseは`AgentRunRepository`がPostgreSQLへ永続化する。Redis-backed一時storeは残さない。
...
- `src/app/application/agent/turn_runner.py`
