...
- 古い入口を新しい業務ユースケースへ移した場合、旧入口は移行完了後に削除する。
- 保存形式を変更する場合、旧形式を読む互換層には削除条件と期限を設定する。
- 派生情報の再構築や修復のためだけに追加された状態は、業務上の意味がなければ残さない。
...
- 個別の互換層、削除期限、移行手順は、この方針ではなく移行記録または運用手順に記録する。
